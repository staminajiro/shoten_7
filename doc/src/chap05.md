...
この技術を使うと、以下のようなムンク風の自画像や浮世絵風の風景写真などを簡単に作ることができます。

![###scale=0.7###](../images/chapter05_face_munch.jpg)
...
今回はスタイル画像として、以下のラーメン二郎の画像を使ってみます。  
![###scale=0.5###](../images/chapter05_jiro.jpg)
...
参考までに、ラーメン二郎風に変換した顔写真、風景写真を載せておきます。  
![###scale=0.7###](../images/chapter05_face_jiro.jpg)
![###scale=0.7###](../images/chapter05_landscape_jiro.jpg)
...
これができれば、例えばゴッホとモネのスタイルを合わせた画像などを、自由に作ることができるようになります。
...
まず、style-imagesに使用したいスタイル画像をカンマ区切りで複数指定します。このとき、カンマの後ろにスペースがあるとエラーとなってしまうのでご注意ください。
...
![###scale=0.7###](../images/chapter05_face_munch_hokusai.jpg)
![###scale=0.7###](../images/chapter05_landscape_munch_hokusai.jpg)

あまり綺麗に変換できませんでしたが、なんとなくムンクのおどろおどろしさと浮世絵の力強さが出ているような気がします。

## まとめ
本章では、軽量型のモデルを使ってJetson NanoでStyle Transferを動かす方法を紹介しました。
また、TensorFlow Liteや量子化を使ってモデルを軽量化したり、複数のスタイルを合成した画像を生成してみました。

Style Transferは既にスマートフォンアプリなどに使われていますが、今回のようにIoTの技術と組み合わせたりすることで、様々なサービスに応用できると考えています。
またディープラーニングによるスタイル変換は、単なる画風変換以外にも動画制作や音声合成など、他の分野でも今後活用が進んでいくのではないでしょうか。
